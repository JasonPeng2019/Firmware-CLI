...
6. **`agent_script_doc_playbook.md`** - Where documentation lives in an MCP product. MCP tools are
   ...
   runs.*
7. **`spec_build_review_loop_playbook.md`** - The team's spec-driven slash-command workflow
   (`/specs` -> `/build` -> `/review`, the `/spec-loop` orchestrator, and the `/fix-bug`, `/test-suite`,
   `/write-process` helpers in `.claude/commands/`). These commands are the playbooks made executable:
   each one loads the authority order, reconcile-before-writing, verify-don't-claim, origin-tag,
   doc-sync, and portability rules and enforces them. *Read when you want to run the workflow, or before
...
## These playbooks are also Agent Skills

Each playbook here is mirrored as a Claude Code Agent Skill under `.claude/skills/superpowers-*/`
(e.g. `superpowers-consistency`, `superpowers-coding`), so the harness can auto-load the right rules by
description when a task matches — not only when an agent remembers to open this index. The playbooks in
this `superpowers/` folder remain the single editing source of truth; the skills' `reference/` copies are
generated from them by `.claude/skills/sync_references.{ps1,sh}` (re-run after editing any playbook), and
`install_skills.{ps1,sh}` makes them reusable in `~/.claude/skills` or another project. The spec-driven
slash commands (`/specs` → `/build` → `/review`, plus `/spec-loop`, `/fix-bug`, `/test-suite`,
`/write-process`) live alongside in `.claude/commands/`. See `spec_build_review_loop_playbook.md` and
