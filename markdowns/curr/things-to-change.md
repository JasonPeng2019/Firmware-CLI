...
Persistent session.
Free host work + final board decision boundary.
Tool schemas.
Batches, wait, UART write.
Progress output + inspector.
Basic timeout hardening.
Client actions / model-authored scripts.
Scoped green approval via model-made flipped tests.
Dynamic timeout refinement and stream checkpoints.
-----

# for full MVP Suite:

Running backlog of changes we want to make but have not committed to a spec yet.
Each entry should say *what* the change is, *where* it belongs in the tree, and
*what it is supposed to do* once landed. Graduate an entry into a real
`markdowns/curr/<step>_spec.md` when it becomes active work.
