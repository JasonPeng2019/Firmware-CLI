...
- Branch C has a restored completion spec and provider-neutral harness path for
  the event spine / timeout-policy surface. Current Windows proof is green for
  `nucleo_l476rg` and retained `nrf52840dk` with both `codex-cli` and
  `claude-cli`, including public `pyocd-debug-brain run` smokes. Official
  `nrf52833dk` remains pending because the attached Nordic board reports
  nRF52840 silicon, and macOS/fresh-host proof remains pending.
