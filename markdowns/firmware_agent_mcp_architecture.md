...
- **Compact memory + native safety sync:** the brain owns structured memory
  ...
  code/workspace context. Remote-primary providers rely on native continuation
  first and receive compact memory as a periodic safety sync, defaulting to
  every 10 provider turns and configurable by CLI flag or environment variable.
