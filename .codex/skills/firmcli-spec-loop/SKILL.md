...
8. If the suite exposes a real bug or repairable coverage gap, route it through `firmcli-fix-bug`, then return to build, review, and suite until the suite is green or the remaining boundary is hardware-only or requires a human decision.
9. Stop when any of these is true:
...
10. Keep the final report in the three verification buckets and include the exact hardware hand-off commands.
