...
1. **`build_plan_concrete.md` is the sacred, living source of truth.** Obey it; when sources disagree,
   it wins.
...
4. **The operating agent is blind to source.** Every runnable script carries a doc that fully replaces
   reading its code.
...
1. **`agent_consistency_playbook.md`** — The authority order and the reconcile-before-writing gate. How
   ...
   what wins.*
2. **`build_plan_concrete.md`** — THE source of truth: architecture, stages (0–8), settled decisions,
...
3. **`agent_coding_guidelines.md`** — How code/docs must be written: prime directive (verify, don't
...
5. **`agent_portability_playbook.md`** — Build for the absent stranger on a fresh macOS/Windows machine:
   ...
   first-run.*
6. **`agent_script_doc_playbook.md`** — Every runnable script gets a doc that fully substitutes for its
   source (the operating agent can't see code): exact behavior, every input, every log/output/exit-code
   meaning, every failure mapped symptom→cause→fix→rerun. *Read when creating or changing any runnable
   script.*
...
a stranger's machine; document every script for an agent that can't see code; and never claim what you
didn't verify.**
