...
BOARD="${BOARD:-nrf52840dk/nrf52840}" # VENDOR-FIXED (Zephyr board target string)
...
case "${APP_DIR} ${CANONICAL_BUILD_DIR}" in
  *" "*)
    # VENDOR-FIXED (current Zephyr/CMake/Kconfig path handling on this host breaks on space-containing
    # paths; build in a scratch dir without spaces, then copy the canonical artifacts back into the repo).
    SCRATCH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/nrf52840dk-build.XXXXXX")"
