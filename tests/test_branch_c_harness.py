    ...
    class FakeProvider:
        ...
        async def next_decision(
            ...
            *,
            ...
            captured["instructions"] = prompt_bundle.system_instructions
            captured["turn_prompt"] = prompt_bundle.turn_context_text
            return ProviderTurn(
...
def test_live_provider_run_accepts_final_result_tool_evidence(monkeypatch) -> None:
    async def fake_run_freeform_task(**kwargs):
        return SimpleNamespace(
            brain_events=(
                {"event_kind": "run_start"},
                {"event_kind": "final_result"},
            ),
            result=SimpleNamespace(mcp_tools_used=["connect", "get_board_info"]),
            state=SimpleNamespace(
                effective_timeout_config=TurnkeyTimeoutConfig(
                    connect_seconds=180.0,
                    flash_seconds=60.0,
                )
            ),
            run_root=None,
    ...
    monkeypatch.setattr(branch_c_tests, "run_freeform_task", fake_run_freeform_task)
    args = branch_c_tests.build_parser().parse_args(
        ["--board-id", "nucleo_l476rg", "--provider", "codex-cli"]
    )

    result = asyncio.run(branch_c_tests._live_provider_run_check(args, "codex-cli"))

    assert result.status == branch_c_tests.PASS
    assert "hardware touched" in result.detail
